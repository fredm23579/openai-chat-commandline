...
function makeSession(title, created, provider, model, messages) {
  return { title, created, provider, model, messages };
}
  ...
  for (const conv of convs) {
    ...
    if (messages.length) {
      sessions.push(makeSession(
        conv.title || 'Imported ChatGPT conversation',
        conv.create_time ? new Date(conv.create_time * 1000).toISOString() : new Date().toISOString(),
...
function importClaude(data) {
  ...
  for (const conv of convs) {
    ...
    if (messages.length) {
      sessions.push(makeSession(
        conv.name || conv.title || 'Imported Claude conversation',
...
function importGemini(data) {
  ...
  for (const conv of convs) {
    ...
    if (messages.length) {
      sessions.push(makeSession(
        conv.title || 'Imported Gemini conversation',
        conv.createTime || conv.create_time || new Date().toISOString(),
        'google', 'gemini-1.5-pro', messages,
      ));
    }
  }
...
function importGenericJson(data) {
  ...
  if (arr[0]?.role && arr[0]?.content) {
    return [makeSession(
      'Imported conversation', new Date().toISOString(), 'openai', 'gpt-4',
      arr.map(m => ({ role: m.role, content: m.content, ts: m.ts || new Date().toISOString() })),
    )];
  }
  ...
  if (arr[0]?.speaker || arr[0]?.sender) {
    ...
    return messages.length
      ? [makeSession('Imported conversation', new Date().toISOString(), 'openai', 'gpt-4', messages)]
      : [];
  }
...
function importMarkdown(text) {
  ...
  for (const line of text.split('\n')) {
    ...
    if (/^#+\s*(user|human|you)\s*:?$/i.test(trimmedLine) || lowerLine === '**user:**') {
      flush(); currentRole = 'user';
    } else if (/^#+\s*(assistant|ai|claude|gpt|gemini|model|bot)\s*:?$/i.test(trimmedLine) || lowerLine === '**assistant:**') {
  ...
  return messages.length
    ? [makeSession('Imported markdown conversation', new Date().toISOString(), 'openai', 'gpt-4', messages)]
    : [];
}
